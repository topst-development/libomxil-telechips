...
OMX_ERRORTYPE base_audio_port_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,omx_base_PortType **openmaxStandPort,OMX_U32 nPortIndex, OMX_BOOL isInput) {
  ...
  if (omx_base_audio_Port->sPortParam.format.audio.cMIMEType != NULL) {
    const char audio_MIME[] = "raw/audio";
    (void)strncpy(omx_base_audio_Port->sPortParam.format.audio.cMIMEType, audio_MIME, sizeof(audio_MIME));
  }
