...
OMX_ERRORTYPE omx_base_component_GetComponentVersion(OMX_IN  OMX_HANDLETYPE hComponent,
  ...
  (void)memcpy(pSpecVersion, &omx_component->nVersion, sizeof(OMX_VERSIONTYPE));
  ...
  uuid[1] = getpid();
  uuid[2] = getuid();
  (void)memcpy(*pComponentUUID, uuid, 3*sizeof(uuid));
...
OMX_ERRORTYPE omx_base_component_GetParameter(
  ...
  case OMX_IndexParamPortDefinition:
    ...
    (void)memcpy(pPortDef, &omx_base_component_Private->ports[pPortDef->nPortIndex]->sPortParam, sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
    break;
  ...
  case OMX_IndexParamPortDefinition: 
    ...
    } 
    {
      ...
      case OMX_PortDomainAudio:
        (void)memcpy(&pPortParam->format.audio, &pPortDef->format.audio, sizeof(OMX_AUDIO_PORTDEFINITIONTYPE));
        break;
      ...
      case OMX_PortDomainOther:
        (void)memcpy(&pPortParam->format.other, &pPortDef->format.other, sizeof(OMX_OTHER_PORTDEFINITIONTYPE));
        break;
      default:
...
OMX_ERRORTYPE omx_base_component_SendCommand(
  ...
  OMX_COMPONENTTYPE* omxComponent = (OMX_COMPONENTTYPE*)hComponent;
  omx_base_component_PrivateType* omx_base_component_Private = (omx_base_component_PrivateType*)omxComponent->pComponentPrivate;
  internalRequestMessageType *message = NULL;
  ...
  if (omx_base_component_Private->state == OMX_StateInvalid) {
    ...
    if (message == NULL) {
      DEBUG(DEB_LEV_FUNCTION_NAME, "message calloc failed!!\n");
  ...
  */
    ...
    if(message->messageParam == OMX_ALL) {
      ...
      for(j = 0; j < NUM_DOMAINS; j++) {
        for(i = omx_base_component_Private->sPortTypesParam[j].nStartPortNumber;
            i < omx_base_component_Private->sPortTypesParam[j].nStartPortNumber +
              omx_base_component_Private->sPortTypesParam[j].nPorts; i++) {
          pthread_mutex_lock(&omx_base_component_Private->flush_mutex);   //CHECKME!!
          omx_base_component_Private->ports[i]->bIsPortFlushed = OMX_TRUE;
          pthread_mutex_unlock(&omx_base_component_Private->flush_mutex); //CHECKME!!
        }
      }
      /* for all ports */
      for(j = 0; j < NUM_DOMAINS; j++) {
  ...
  case OMX_CommandPortDisable: {
    ...
    if(message->messageParam == OMX_ALL) {
      ...
      pPort=omx_base_component_Private->ports[message->messageParam];
      if(omx_base_component_Private->state!=OMX_StateLoaded) {
        (void)pPort->FlushProcessingBuffers(pPort);
