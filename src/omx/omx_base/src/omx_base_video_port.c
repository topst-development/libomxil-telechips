...
OMX_ERRORTYPE base_video_port_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,omx_base_PortType **openmaxStandPort,OMX_U32 nPortIndex, OMX_BOOL isInput) {
  ...
  if (omx_base_video_Port->sPortParam.format.video.cMIMEType != NULL) {
    char video_MIME[] = "raw/video";
    (void)strncpy(omx_base_video_Port->sPortParam.format.video.cMIMEType, video_MIME, sizeof(video_MIME));
  }
